                ...
                new Thread(() -> {
                    try (Scanner input = wrapper.getInput(); PrintWriter output = wrapper.getOutput()) {
                        ...
                        while (true) {
                            ...
                            String recipient = clientInput.substring(0, 2);
                            SocketWrapper sender = clients.get(clientId);
                            if (recipient.charAt(0) == '@') {
                                if (recipient.charAt(1) == ' ') {
                                    sender.getOutput().println("Сообщение не отправлено");
                                } else {
                                    long destinationId = Long.parseLong(recipient.substring(1, 2));
                                    SocketWrapper destination = clients.get(destinationId);
                                    destination.getOutput().println(clientInput);
                                }
                            } else {
                                clients.values().stream().filter(it -> it.getId() != sender.getId()).forEach(it -> it.getOutput().println(clientId + "- " + clientInput));
